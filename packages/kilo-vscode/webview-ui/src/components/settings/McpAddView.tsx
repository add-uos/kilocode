...
const McpAddView: Component<Props> = (props) => {
  ...
  const submit = () => {
    ...
    const mcp: McpConfig = transport() === "stdio" ? { command: command().trim().split(/\s+/) } : { url: url().trim() }
    ...
    <div>
      ...
      <Show when={transport() === "stdio"}>
        <Card style={{ "margin-bottom": "12px" }}>
          ...
          <div data-slot="settings-row-label-subtitle" style={{ "margin-bottom": "8px" }}>
            {language.t("settings.agentBehaviour.addMcp.command.help")}
          </div>
          <TextField
            value={command()}
      ...
      <Show when={transport() === "sse"}>
        <Card style={{ "margin-bottom": "12px" }}>
          ...
          <div data-slot="settings-row-label-subtitle" style={{ "margin-bottom": "8px" }}>
            {language.t("settings.agentBehaviour.addMcp.url.help")}
          </div>
          <TextField
            value={url()}
