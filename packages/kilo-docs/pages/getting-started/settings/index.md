...
Kilo Code offers multiple platforms, each with its own approach to configuration. The **VSCode (Legacy)** version uses the VS Code settings UI with export/import/reset workflows. The current **VSCode** extension provides a Settings webview that reads and writes to JSONC config files. The **CLI** uses portable JSONC config files directly.
