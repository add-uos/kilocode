...
In the Classic extension, orchestrator mode uses two tools to manage subtasks:
...
- The `task` tool creates a new child session. The agent name is specified as a parameter (e.g., `general`, `explore`, `docs`).
